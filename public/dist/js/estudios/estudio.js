$(document).ready(function(){
                        ...
                        $('#tabla_proc_comp tbody').append(
                            '<tr><td hidden>' + value.id + '</td>'+
                                ...
                                '<td class="text-center">'+
                                    '<a href="javascript:void(0);" data-id="'+ value.id+ '" data-route="{{ route("storeDetalleProc") }} " class="btn btn-sm btn-outline-info btn-add-asp" title="Agregar Aspectos"><i class="fas fa-cogs"></i></a>'+
                                    '<a href="javascript:void(0);" data-id="'+ value.id+ '" data-route="{{ route("storeDetalleProc") }} " class="btn btn-sm btn-outline-danger btn-del-comp" title="Eliminar Componente"><i class="fas fa-trash-alt"></i></a>'+
                                '</td>'+
                            '</tr>');
                    });
                }else {
                ...
                if (data.length != 0) {
                    ...
                    $.each(data, function(index, value) {
                        $('.tabla_componentes tbody').append(
                            '<tr><td hidden>' + value.id + '</td>'+
                                '<td>' + value.nombre + '</td>'+
                                '<td class="text-center"><a href="javascript:void(0);" data-id="'+ value.id+ '" data-route="{{ route("storeDetalleProc") }} " class="btn btn-sm btn-outline-success btn-use-comp" title="Usar Componente"><i class="fas fa-plus-circle fa-lg"></i></a></td>'+
                            '</tr>');
                    });
                }else {
    ...
    $(document).on('click', '.btn-use-comp', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                addComponenteDP(dp_id, 'asd', com_id);
            }
        });
    });
